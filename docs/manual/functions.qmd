...
| ARABIC(Text) | Converts a Roman numeral (*Text*) to a number, like the *Excel* function.<br>\linebreak *Text* is case-insensitive.<br>\linebreak An invalid or malformed numeral returns NaN.<br>\linebreak `ARABIC("MCMXCIX")` = 1999 |
...
| NUMBERVALUE(Text, DecimalSeparator, GroupSeparator) | Converts *Text* to a number, independent of the current locale.<br>\linebreak *DecimalSeparator* and *GroupSeparator* are optional and default to `.` and `,`. If only *DecimalSeparator* is given, then *GroupSeparator* defaults to whichever of `.` and `,` it is not.<br>\linebreak Only the first character of a separator is used, and the two cannot be the same.<br>\linebreak Trailing percent signs are cumulative, so `NUMBERVALUE("9%%")` = 0.0009.<br>\linebreak An empty *Text* returns `0`.<br>\linebreak A group separator before the decimal separator is ignored. After it, NaN is returned.<br>\linebreak NaN is also returned if the decimal separator appears more than once, or if *Text* does not fully convert.<br>\linebreak Spaces at the start and end of *Text* are ignored, but a space inside it is not (refer to the warning below).<br>\linebreak `NUMBERVALUE("2.500,27", ",", ".")` = 2500.27<br>\linebreak `NUMBERVALUE("1,234.56")` = 1234.56<br>\linebreak `NUMBERVALUE("3.5%")` = 0.035<br>\linebreak `NUMBERVALUE("3 000", ".", " ")` = 3000 |
...
`ARABIC()` is stricter than some spreadsheet programs, which accept malformed Roman numerals.
For example, other programs may return `99` for `ARABIC("IC")`, even though `I` cannot subtract from `C`.
*TinyExpr++* returns NaN for that, since it is not a properly formed Roman numeral.
A numeral is only accepted if:

- It uses only the letters `I`, `V`, `X`, `L`, `C`, `D`, and `M` (case-insensitive).
- `I`, `X`, and `C` repeat no more than four times in a row (e.g., `IIII`, `XXXX`, `CCCC`).
  This is an additive form for 4, 40, and 400 that remains common alongside the subtractive one.
  `M` repeats no more than three times, since a fourth would exceed the largest representable numeral, `3999` (`MMMCMXCIX`).
  `V`, `L`, and `D` never repeat.
- Subtraction is only allowed for `I` before `V`/`X`, `X` before `L`/`C`, and `C` before `D`/`M`.
- A repeated numeral cannot immediately precede a subtractive pair (e.g., `IIX`).
:::
...
::: {.warningsection data-latex=""}
`NUMBERVALUE()` only ignores the spaces around *Text*, while *Excel* ignores them anywhere in it.
...
Pass the space as *GroupSeparator* instead:

```
NUMBERVALUE("3 000")             // NaN
NUMBERVALUE("3 000", ".", " ")   // 3000
```

(Trailing spaces are still removed, so `NUMBERVALUE("3.5 %")` is 0.035.)
:::

::: {.notesection data-latex=""}
Separators are a single `char`, so a *UTF-8* non-breaking space (`0xC2 0xA0`) cannot be used as one.
Convert such spaces to a regular one before compiling the formula.
:::
...
| Function | Description |
| :-- | :-- |
| BIN2DEC(Text) | Converts a binary string (*Text*) to a number, like the *Excel* function.<br>\linebreak *Text* may be up to 10 digits and is read as two's complement.<br>\linebreak `BIN2DEC("1111111111")` = -1 |
...
| HEX2DEC(Text) | Converts a hexadecimal string (*Text*) to a number, like the *Excel* function.<br>\linebreak *Text* may be up to 10 digits and is read as two's complement.<br>\linebreak `HEX2DEC("FFFFFFFFFF")` = -1 |
| OCT2DEC(Text) | Converts an octal string (*Text*) to a number, like the *Excel* function.<br>\linebreak *Text* may be up to 10 digits and is read as two's complement.<br>\linebreak `OCT2DEC("7777777777")` = -1 |
...
::: {.warningsection data-latex=""}
Defining `TE_FLOAT` will disable all bitwise functions and operators, along with `BIN2DEC()`, `HEX2DEC()`, and `OCT2DEC()`.
:::
...
| Function | Description |
| :-- | :-- |
| CUMIPMT(Rate, Periods, PresentValue, StartPeriod, EndPeriod, Type) | Returns the cumulative interest paid on a loan between *StartPeriod* and *EndPeriod*, inclusive.<br>\linebreak *Type* specifies when payments are due: 0 = end of period, 1 = beginning of period.<br>\linebreak NaN will be returned if *Rate* <= 0, *Periods* <= 0, *PresentValue* <= 0, if *StartPeriod* or *EndPeriod* are out of range, or if *Type* is not 0 or 1. |
| CUMPRINC(Rate, Periods, PresentValue, StartPeriod, EndPeriod, Type) | Returns the cumulative principal paid on a loan between *StartPeriod* and *EndPeriod*, inclusive.<br>\linebreak Note that *Type* specifies when payments are due: 0 = end of period, 1 = beginning of period.<br>\linebreak NaN will be returned if *Rate* <= 0, *Periods* <= 0, *PresentValue* <= 0, if *StartPeriod* or *EndPeriod* are out of range, or if *Type* is not 0 or 1. |
...
| PPMT(Rate, Period, Periods, PresentValue, [FutureValue], [Type]) | Returns the principal portion of a payment for a specified period of an investment.<br>\linebreak Note that *FutureValue* and *Type* are optional and default to 0.<br>\linebreak Also, *Type* specifies when payments are due: 0 = end of period, 1 = beginning of period.<br>\linebreak NaN will be returned if arguments are invalid or out of range. |
